...
Installing by hand works the same as any other component: copy the file from
the Manual tab and adjust the import paths. The animations travel inside the
file as `SPRITE_CSS`, so nothing else needs copying — either render with
`withStyles` or inject the string once:
...
file, a README. On this site every sprite is also a URL:

```
/api/sprite/{seed}.svg?state=loop&size=96
```

The same seed always answers with the same image, so the response is cached as
immutable.
